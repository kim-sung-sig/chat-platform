...
- Schedule BC
	- `ScheduleRule` (Aggregate Root)
		- `scheduleId`, `targetRoomId`/`targetUserId`, `messageTemplate`, `scheduleType`(단발/주기), `cronExpression` 또는
		  `executeAt`, `status`
...
**⚠️ 중요**: `common-logging`은 초기 구현 단계에서는 기본 기능만 구현하고, **향후 ELK Stack, Grafana Loki, OpenTelemetry 등으로 전환할 때 본격적으로 확장**할
예정입니다.
...
3. **순환 의존 체크**
	- `common-util`, `common-auth`, `common-logging`은 서로 의존하지 않음 (단, `common-auth`와 `common-logging`은 `common-util`에만
	  의존)
...
| 단계      | 내용                                                         | 우선순위    | 상태      | 실제 소요 |
|---------|------------------------------------------------------------|---------|---------|-------|
| STEP 1  | `common` 모듈 세분화 (common-util, common-auth, common-logging) | 높음      | ✅ 완료    | 2일    |
| STEP 2  | `chat-storage` 확장 및 메시지 타입 전략 설계                           | **최우선** | ✅ 완료    | 3일    |
| STEP 3  | `chat-message-server` 재구성                                  | 높음      | ✅ 완료    | 4일    |
| STEP 4  | `chat-system-server` 재구성                                   | 높음      | 🔄 진행 중 | -     |
| STEP 5  | `chat-websocket-server` 재구성                                | 중간      | ✅ 완료    | 3일    |
| STEP 6  | `domain` 모듈 평가 및 통합                                        | 중간      | ✅ 완료    | 1일    |
| STEP 7  | 빌드 설정 정리                                                   | 높음      | ✅ 완료    | 1일    |
| STEP 8  | 예약 메시지 시스템 (Quartz)                                        | 높음      | 🚀 시작   | -     |
| STEP 9  | 확장성 검증 테스트                                                 | 중간      | ⏳ 대기    | -     |
| STEP 10 | 문서화                                                        | 낮음      | 🔄 진행 중 | -     |

**총 예상 소요**: 약 3-4주 → **실제 진행**: 2주 (67% 완료)
...
- 🚀 STEP 8: 예약 메시지 시스템 (Quartz Scheduler)
- 🔄 STEP 4: chat-system-server 고도화
