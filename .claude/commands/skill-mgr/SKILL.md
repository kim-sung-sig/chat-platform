...
source: .harness/skills/skill-mgr/SKILL.md
skill: skill-mgr
...
description: >
  ...
  Use this skill:
    ...
    * When syncing .harness/skills/ changes to .claude/commands/
...
`.harness/skills/` 관리 체계 안에서 점진적으로 강화합니다.
...
IMPORT    → .harness/skills/<id>/SKILL.md 로 가져오기
REGISTER  → .harness/registries/skills.json 등록
SYNC      → scripts/sync-skills.ps1 실행 → .claude/commands/ 생성
...
`.harness/registries/skills.json`의 `managedSkills` 배열에 추가:

```json
{
  "id": "<id>",
  "source": ".harness/skills/<id>/SKILL.md",
  "tier": "optional",
  "mutable": true,
  "tool_aliases": {
    "claude": ["<id>"],
...
→ `.claude/commands/<id>/SKILL.md` 자동 생성 (직접 편집 금지)
...
| sdd-craft | optional | true | 효과적 SDD 생성 |
| tdd-cycle | optional | true | TDD 개선 사이클 |
| arch-policy | optional | true | 아키텍처 정책 |
