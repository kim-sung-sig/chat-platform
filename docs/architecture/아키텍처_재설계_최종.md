# 채팅 플랫폼 아키텍처 재설계 (최종)

## 📋 목차

1. [문제점 분석](#문제점-분석)
2. [재설계 방향](#재설계-방향)
3. [모듈 구조](#모듈-구조)
4. [의존성 다이어그램](#의존성-다이어그램)
5. [각 모듈 상세 설계](#각-모듈-상세-설계)
6. [구현 전략](#구현-전략)
7. [마이그레이션 단계](#마이그레이션-단계)

---

## 🔍 문제점 분석

### 현재 구조의 문제

1. **JPA 설정 누락**: `chat-storage`에 JPA가 있지만 사용하는 모듈에서 EntityScan 설정 누락
2. **도메인 로직 분산**: 도메인 서비스가 각 서버 모듈에 분산되어 중복 발생 가능
3. **책임 불명확**: Entity만 있는 storage vs 도메인 로직을 가진 서버 모듈 간 경계 모호
4. **의존성 혼란**: 어떤 모듈이 어떤 책임을 가져야 하는지 불명확

---

## 🎯 재설계 방향

### 핵심 원칙

1. **DDD 계층 분리**: Domain Layer와 Infrastructure Layer 명확히 분리
2. **Hexagonal Architecture**: 도메인을 중심으로, 포트/어댑터 패턴 적용
3. **단일 책임**: 각 모듈은 명확한 하나의 책임만 가짐
4. **의존성 역전**: 도메인은 인프라에 의존하지 않음

### 새로운 모듈 전략

- **chat-domain**: 순수 도메인 모델 + 도메인 서비스 + Repository Interface
- **chat-storage**: JPA 구현 + Repository 구현체 (chat-domain에 의존)
- **서버 모듈들**: Application Layer (Use Case) + Presentation Layer

---

## 📦 모듈 구조

```
chat-platform/
│
├── common/                        # 공통 라이브러리
│   ├── common-util/               # 예외, 상수, 유틸리티
│   ├── common-auth/               # 인증/인가 (JWT, Security)
│   └── common-logging/            # 로깅 (향후 ELK 연동)
│
├── chat-domain/                   # 도메인 계층 (핵심 비즈니스)
│   ├── domain/
│   │   ├── channel/               # 채널 도메인 모델
│   │   │   ├── Channel.java
│   │   │   ├── ChannelId.java
│   │   │   ├── ChannelType.java
│   │   │   └── ChannelRepository.java (interface)
│   │   ├── message/               # 메시지 도메인 모델
│   │   │   ├── Message.java
│   │   │   ├── MessageId.java
│   │   │   ├── MessageType.java
│   │   │   ├── MessageStatus.java (enum)
│   │   │   ├── MessageContent.java
│   │   │   └── MessageRepository.java (interface)
│   │   ├── schedule/              # 스케줄 도메인 모델
│   │   │   ├── ScheduleRule.java
│   │   │   ├── ScheduleType.java (enum: ONE_TIME, RECURRING)
│   │   │   ├── ScheduleStatus.java (enum)
│   │   │   └── ScheduleRuleRepository.java (interface)
│   │   └── user/                  # 사용자 도메인 모델
│   │       ├── User.java
│   │       └── UserId.java
│   └── service/                   # 도메인 서비스
│       ├── MessageDomainService.java
│       ├── ChannelDomainService.java
│       └── ScheduleDomainService.java
│
├── chat-storage/                  # 영속성 계층 (Infrastructure)
│   ├── entity/                    # JPA Entity
│   │   ├── ChatChannelEntity.java
│   │   ├── ChatMessageEntity.java
│   │   ├── MessageReadEntity.java
│   │   ├── ScheduleRuleEntity.java
│   │   └── OutboxEventEntity.java
│   ├── repository/                # JPA Repository 구현
│   │   ├── JpaChatChannelRepository.java
│   │   ├── JpaChatMessageRepository.java
│   │   ├── JpaScheduleRuleRepository.java
│   │   └── ...
│   ├── adapter/                   # Domain Repository 구현 (Adapter)
│   │   ├── ChannelRepositoryAdapter.java
│   │   ├── MessageRepositoryAdapter.java
│   │   └── ScheduleRepositoryAdapter.java
│   ├── mapper/                    # Entity ↔ Domain Model 매핑
│   │   ├── ChannelMapper.java
│   │   ├── MessageMapper.java
│   │   └── ScheduleMapper.java
│   └── config/
│       └── JpaConfig.java
│
├── chat-message-server/           # 메시지 발송 서버
│   ├── application/               # Use Case (Application Service)
│   │   ├── SendMessageUseCase.java
│   │   ├── PublishMessageUseCase.java
│   │   └── MessageReadUseCase.java
│   ├── presentation/              # REST Controller
│   │   ├── MessageController.java
│   │   └── MessageReadController.java
│   ├── infrastructure/            # Redis, Event Publisher
│   │   ├── RedisMessagePublisher.java
│   │   └── MessageEventPublisher.java
│   └── config/
│       ├── SecurityConfig.java
│       └── RedisConfig.java
│
├── chat-system-server/            # 시스템 관리 서버
│   ├── application/               # Use Case
│   │   ├── channel/
│   │   │   ├── CreateChannelUseCase.java
│   │   │   ├── JoinChannelUseCase.java
│   │   │   └── LeaveChannelUseCase.java
│   │   ├── message/
│   │   │   └── QueryMessageUseCase.java (Cursor 기반 페이징)
│   │   └── schedule/
│   │       ├── CreateScheduleUseCase.java
│   │       ├── CancelScheduleUseCase.java
│   │       └── ForcePublishScheduleUseCase.java
│   ├── presentation/              # REST Controller
│   │   ├── ChannelController.java
│   │   ├── MessageQueryController.java
│   │   └── ScheduleController.java
│   ├── infrastructure/
│   │   ├── scheduler/             # Quartz 스케줄러
│   │   │   ├── MessagePublishJob.java
│   │   │   ├── QuartzSchedulerService.java
│   │   │   └── JobConcurrencyGuard.java
│   │   └── client/
│   │       └── MessageServerClient.java (REST 호출)
│   └── config/
│       ├── QuartzConfig.java
│       └── RestTemplateConfig.java
│
└── chat-websocket-server/         # WebSocket 서버
    ├── application/
    │   └── WebSocketMessageHandler.java
    ├── infrastructure/
    │   ├── session/
    │   │   └── WebSocketSessionManager.java (Redis 기반)
    │   └── subscriber/
    │       └── RedisMessageSubscriber.java
    └── config/
        ├── WebSocketConfig.java
        └── RedisSubscriberConfig.java
```

---

## 🔗 의존성 다이어그램

```
                    ┌─────────────┐
                    │ common-util │
                    └──────┬──────┘
                           │
              ┌────────────┴────────────┐
              │                         │
       ┌──────▼──────┐         ┌───────▼────────┐
       │ common-auth │         │ common-logging │
       └─────────────┘         └────────────────┘
              │
              │
       ┌──────▼──────────┐
       │  chat-domain    │  ◄── 순수 도메인 (의존성 최소)
       │ (domain models, │
       │  services,      │
       │  interfaces)    │
       └──────┬──────────┘
              │
       ┌──────▼──────────┐
       │  chat-storage   │  ◄── 영속성 구현 (JPA)
       │ (entities,      │
       │  repositories)  │
       └──────┬──────────┘
              │
    ┌─────────┴─────────┬─────────────────┐
    │                   │                 │
┌───▼──────────────┐ ┌──▼─────────────┐ ┌▼──────────────────┐
│ chat-message-    │ │ chat-system-   │ │ chat-websocket-   │
│ server           │ │ server         │ │ server            │
│ (메시지 발송)     │ │ (시스템 관리)   │ │ (실시간 통신)      │
└──────────────────┘ └────────────────┘ └───────────────────┘
```

### 의존성 규칙

1. **Domain → 아무것도 의존하지 않음** (common-util만 허용)
2. **Storage → Domain 의존**
3. **Server 모듈들 → Domain, Storage 의존**
4. **Server 모듈 간 직접 의존 금지** (REST API나 이벤트로 통신)

---

## 📝 각 모듈 상세 설계

### 1. chat-domain (도메인 계층)

#### 목적

- 순수한 비즈니스 로직과 도메인 모델 정의
- 인프라(DB, 프레임워크)에 독립적

#### 주요 구성요소

**1.1 Domain Models (Aggregate Root)**

```java
// Message Aggregate
public class Message {
    private MessageId id;
    private ChannelId channelId;
    private UserId senderId;
    private MessageContent content;
    private MessageType type;
    private MessageStatus status;  // ENUM: PENDING, SENT, DELIVERED, READ, FAILED
    private Instant createdAt;
    
    // 도메인 로직
    public void markAsSent() { ... }
    public void markAsDelivered() { ... }
    public void markAsRead(UserId userId) { ... }
    public boolean canBeEdited() { ... }
}

// Schedule Aggregate
public class ScheduleRule {
    private ScheduleId id;
    private ScheduleType type;  // ENUM: ONE_TIME, RECURRING
    private ScheduleStatus status;  // ENUM: PENDING, ACTIVE, EXECUTED, CANCELLED
    private Message message;
    private CronExpression cronExpression;  // for RECURRING
    private Instant scheduledAt;  // for ONE_TIME
    
    // 도메인 로직
    public void cancel() { ... }
    public void markAsExecuted() { ... }
    public boolean canBeExecuted() { ... }
}
```

**1.2 Repository Interfaces (Port)**

```java
public interface MessageRepository {
    Message save(Message message);
    Optional<Message> findById(MessageId id);
    List<Message> findByChannelId(ChannelId channelId, Cursor cursor, int limit);
}

public interface ScheduleRuleRepository {
    ScheduleRule save(ScheduleRule schedule);
    Optional<ScheduleRule> findById(ScheduleId id);
    Optional<ScheduleRule> findByIdWithLock(ScheduleId id);
}
```

**1.3 Domain Services**

```java
@Service
public class MessageDomainService {
    public Message createMessage(ChannelId channelId, UserId senderId, MessageContent content) {
        // 도메인 규칙 검증
        validateMessageContent(content);
        return Message.create(channelId, senderId, content);
    }
    
    public void validateMessageContent(MessageContent content) {
        // 비즈니스 규칙
    }
}

@Service
public class ScheduleDomainService {
    public ScheduleRule createOneTimeSchedule(Message message, Instant scheduledAt) {
        return ScheduleRule.oneTime(message, scheduledAt);
    }
    
    public ScheduleRule createRecurringSchedule(Message message, CronExpression cron) {
        return ScheduleRule.recurring(message, cron);
    }
}
```

#### 의존성

```gradle
dependencies {
    implementation project(':common-util')
    // 외부 의존성 최소화
}
```

---

### 2. chat-storage (영속성 계층)

#### 목적

- Domain Repository Interface의 구현
- JPA Entity와 Domain Model 간 변환
- 데이터베이스 접근 추상화

#### 주요 구성요소

**2.1 JPA Entities**

```java
@Entity
@Table(name = "chat_messages")
public class ChatMessageEntity {
    @Id
    private String id;
    
    @Column(name = "channel_id")
    private String channelId;
    
    @Column(name = "sender_id")
    private String senderId;
    
    @Enumerated(EnumType.STRING)
    private MessageStatus status;
    
    @Enumerated(EnumType.STRING)
    private MessageType type;
    
    // ... JPA 전용 필드
}
```

**2.2 Repository Implementations (Adapter)**

```java
@Repository
@RequiredArgsConstructor
public class MessageRepositoryAdapter implements MessageRepository {
    private final JpaChatMessageRepository jpaRepository;
    private final MessageMapper mapper;
    
    @Override
    public Message save(Message message) {
        ChatMessageEntity entity = mapper.toEntity(message);
        ChatMessageEntity saved = jpaRepository.save(entity);
        return mapper.toDomain(saved);
    }
    
    @Override
    public Optional<Message> findById(MessageId id) {
        return jpaRepository.findById(id.getValue())
            .map(mapper::toDomain);
    }
}
```

**2.3 Mappers**

```java
@Component
public class MessageMapper {
    public ChatMessageEntity toEntity(Message domain) {
        // Domain → Entity 변환
    }
    
    public Message toDomain(ChatMessageEntity entity) {
        // Entity → Domain 변환
    }
}
```

**2.4 JPA Configuration**

```java
@Configuration
@EnableJpaRepositories(basePackages = "com.example.chat.storage.repository")
@EntityScan(basePackages = "com.example.chat.storage.entity")
public class JpaConfig {
    // JPA 설정
}
```

#### 의존성

```gradle
dependencies {
    implementation project(':common-util')
    implementation project(':chat-domain')
    
    implementation 'org.springframework.boot:spring-boot-starter-data-jpa'
    implementation 'org.springframework.boot:spring-boot-starter-validation'
    runtimeOnly 'org.postgresql:postgresql'
}
```

---

### 3. chat-message-server (메시지 발송 서버)

#### 목적

- 메시지 발송 API 제공
- 메시지 저장 및 Redis Pub/Sub 발행

#### 주요 Use Cases

```java
@Service
@RequiredArgsConstructor
public class SendMessageUseCase {
    private final MessageRepository messageRepository;
    private final MessageDomainService messageDomainService;
    private final ChannelRepository channelRepository;
    private final RedisMessagePublisher redisPublisher;
    
    @Transactional
    public MessageResponse execute(SendMessageCommand command) {
        // 1. 채널 조회
        Channel channel = channelRepository.findById(command.channelId())
            .orElseThrow(() -> new ChannelNotFoundException(command.channelId()));
        
        // 2. 권한 검증 (Early Return)
        if (!channel.isMember(command.senderId())) {
            throw new UnauthorizedAccessException("Not a member");
        }
        
        // 3. 도메인 서비스를 통한 메시지 생성
        Message message = messageDomainService.createMessage(
            command.channelId(),
            command.senderId(),
            command.content()
        );
        
        // 4. 저장
        Message saved = messageRepository.save(message);
        
        // 5. Redis 발행
        redisPublisher.publish(saved);
        
        // 6. 응답
        return MessageResponse.from(saved);
    }
}
```

#### 의존성

```gradle
dependencies {
    implementation project(':common-util')
    implementation project(':common-auth')
    implementation project(':common-logging')
    implementation project(':chat-domain')
    implementation project(':chat-storage')
    
    implementation 'org.springframework.boot:spring-boot-starter-web'
    implementation 'org.springframework.boot:spring-boot-starter-data-redis'
    implementation 'org.springframework.boot:spring-boot-starter-security'
}
```

---

### 4. chat-system-server (시스템 관리 서버)

#### 목적

- 채팅방 관리 (생성, 참여, 조회)
- 메시지 조회 (Cursor 기반 페이징)
- 예약 메시지 관리 (Quartz 스케줄러)

#### 주요 Use Cases

**4.1 메시지 조회 (Cursor 기반)**

```java
@Service
@RequiredArgsConstructor
public class QueryMessageUseCase {
    private final MessageRepository messageRepository;
    private final ChannelRepository channelRepository;
    
    @Transactional(readOnly = true)
    public MessagePageResponse execute(QueryMessageCommand command) {
        // 1. 채널 조회
        Channel channel = channelRepository.findById(command.channelId())
            .orElseThrow(() -> new ChannelNotFoundException(command.channelId()));
        
        // 2. 권한 검증
        if (!channel.isMember(command.userId())) {
            throw new UnauthorizedAccessException("Not a member");
        }
        
        // 3. Cursor 기반 조회
        List<Message> messages = messageRepository.findByChannelId(
            command.channelId(),
            command.cursor(),
            command.limit()
        );
        
        // 4. 다음 커서 생성
        Cursor nextCursor = messages.isEmpty() ? null : 
            Cursor.from(messages.get(messages.size() - 1));
        
        return MessagePageResponse.of(messages, nextCursor);
    }
}
```

**4.2 예약 메시지 (단발성)**

```java
@Service
@RequiredArgsConstructor
public class CreateOneTimeScheduleUseCase {
    private final ScheduleRuleRepository scheduleRepository;
    private final MessageDomainService messageDomainService;
    private final ScheduleDomainService scheduleDomainService;
    private final QuartzSchedulerService quartzService;
    
    @Transactional
    public ScheduleResponse execute(CreateOneTimeScheduleCommand command) {
        // 1. 메시지 생성 (도메인 서비스)
        Message message = messageDomainService.createMessage(
            command.channelId(),
            command.senderId(),
            command.content()
        );
        
        // 2. 스케줄 생성 (도메인 서비스)
        ScheduleRule schedule = scheduleDomainService.createOneTimeSchedule(
            message,
            command.scheduledAt()
        );
        
        // 3. 저장
        ScheduleRule saved = scheduleRepository.save(schedule);
        
        // 4. Quartz Job 등록
        quartzService.scheduleOneTimeJob(saved);
        
        return ScheduleResponse.from(saved);
    }
}
```

**4.3 예약 메시지 (주기적)**

```java
@Service
@RequiredArgsConstructor
public class CreateRecurringScheduleUseCase {
    private final ScheduleRuleRepository scheduleRepository;
    private final MessageDomainService messageDomainService;
    private final ScheduleDomainService scheduleDomainService;
    private final QuartzSchedulerService quartzService;
    
    @Transactional
    public ScheduleResponse execute(CreateRecurringScheduleCommand command) {
        // 1. 메시지 템플릿 생성
        Message messageTemplate = messageDomainService.createMessage(
            command.channelId(),
            command.senderId(),
            command.content()
        );
        
        // 2. 주기 스케줄 생성
        ScheduleRule schedule = scheduleDomainService.createRecurringSchedule(
            messageTemplate,
            command.cronExpression()
        );
        
        // 3. 저장
        ScheduleRule saved = scheduleRepository.save(schedule);
        
        // 4. Quartz Cron Job 등록
        quartzService.scheduleRecurringJob(saved);
        
        return ScheduleResponse.from(saved);
    }
}
```

**4.4 강제 발송 (동시성 제어)**

```java
@Service
@RequiredArgsConstructor
public class ForcePublishScheduleUseCase {
    private final ScheduleRuleRepository scheduleRepository;
    private final MessageServerClient messageClient;
    private final QuartzSchedulerService quartzService;
    
    @Transactional
    public void execute(ForcePublishCommand command) {
        // 1. 비관적 락으로 스케줄 조회
        ScheduleRule schedule = scheduleRepository
            .findByIdWithLock(command.scheduleId())
            .orElseThrow(() -> new ScheduleNotFoundException(command.scheduleId()));
        
        // 2. 상태 검증 (Early Return)
        if (!schedule.canBeExecuted()) {
            throw new InvalidScheduleStateException("Cannot execute schedule");
        }
        
        // 3. 단발성인 경우 Quartz Job 삭제
        if (schedule.isOneTime()) {
            quartzService.deleteJob(schedule.getId());
        }
        
        // 4. 메시지 발송 (chat-message-server 호출)
        messageClient.sendMessage(schedule.getMessage());
        
        // 5. 상태 업데이트
        schedule.markAsExecuted();
        scheduleRepository.save(schedule);
    }
}
```

#### 동시성 제어 전략

**단발성 메시지**

- DB 비관적 락 (`SELECT ... FOR UPDATE`)
- Message 상태로 중복 실행 방지
- Quartz Job 실행 시 상태 체크

**주기적 메시지**

- DB 비관적 락으로 ScheduleRule 보호
- 메시지는 매번 새로 생성 (MessageId 중복 없음)
- Quartz Job에서 `@DisallowConcurrentExecution` 사용

```java
@DisallowConcurrentExecution
@Component
public class MessagePublishJob implements Job {
    @Override
    public void execute(JobExecutionContext context) {
        // ...
    }
}
```

#### 의존성

```gradle
dependencies {
    implementation project(':common-util')
    implementation project(':common-auth')
    implementation project(':common-logging')
    implementation project(':chat-domain')
    implementation project(':chat-storage')
    
    implementation 'org.springframework.boot:spring-boot-starter-web'
    implementation 'org.springframework.boot:spring-boot-starter-quartz'
    implementation 'org.springframework.boot:spring-boot-starter-security'
}
```

---

### 5. chat-websocket-server (WebSocket 서버)

#### 목적

- WebSocket 연결 관리
- Redis Pub/Sub 구독 → WebSocket 전송

#### 주요 구성요소

```java
@Component
@RequiredArgsConstructor
public class RedisMessageSubscriber {
    private final WebSocketSessionManager sessionManager;
    
    @RedisListener(topic = "chat.message")
    public void onMessage(MessageEvent event) {
        // 채널의 모든 세션에 전송
        sessionManager.sendToChannel(event.getChannelId(), event);
    }
}

@Component
public class WebSocketSessionManager {
    private final RedisTemplate<String, String> redisTemplate;
    
    // Redis 기반 세션 관리 (멀티 인스턴스 대응)
    public void registerSession(String sessionId, ChannelId channelId) { ... }
    public void sendToChannel(ChannelId channelId, MessageEvent event) { ... }
}
```

#### 의존성

```gradle
dependencies {
    implementation project(':common-util')
    implementation project(':common-auth')
    implementation project(':chat-domain')  // 읽기 전용
    
    implementation 'org.springframework.boot:spring-boot-starter-websocket'
    implementation 'org.springframework.boot:spring-boot-starter-data-redis'
}
```

---

## 🚀 구현 전략

### Phase 1: Domain 계층 구축

1. `chat-domain` 모듈 생성
2. Aggregate Root 정의 (Message, Channel, ScheduleRule)
3. Repository Interface 정의
4. Domain Service 구현
5. Value Objects 및 Enums 정의

### Phase 2: Storage 계층 구축

1. `chat-storage` 모듈 수정
2. JPA Entity 생성/수정
3. Repository Adapter 구현
4. Mapper 구현
5. JPA Configuration 설정

### Phase 3: 서버 모듈 리팩토링

1. `chat-message-server` Use Case 구현
2. `chat-system-server` Use Case 구현
3. Controller 레이어 정리
4. 각 서버의 Infrastructure 구현

### Phase 4: 통합 및 테스트

1. 통합 테스트 작성
2. API 문서화
3. 성능 테스트

---

## 📋 마이그레이션 단계

### Step 1: chat-domain 모듈 생성

```bash
# 1. 디렉토리 생성
mkdir -p chat-domain/src/main/java/com/example/chat/domain

# 2. build.gradle 생성
# 3. Domain Models 이관
# 4. Repository Interfaces 정의
```

### Step 2: chat-storage 리팩토링

```bash
# 1. 기존 Entity 검토 및 정리
# 2. Adapter 패키지 생성
# 3. Mapper 구현
# 4. JpaConfig 수정 (EntityScan, EnableJpaRepositories)
```

### Step 3: 서버 모듈 리팩토링

```bash
# 1. Application 패키지 생성 (Use Case)
# 2. 기존 Service → Use Case로 변경
# 3. Domain 의존성 추가
# 4. 불필요한 코드 제거
```

### Step 4: 의존성 정리

```bash
# 1. 각 모듈의 build.gradle 수정
# 2. settings.gradle에 chat-domain 추가
# 3. 순환 의존성 제거
```

### Step 5: 테스트 및 검증

```bash
# 1. 단위 테스트 작성
# 2. 통합 테스트 실행
# 3. API 테스트
```

---

## 📊 기대 효과

### 1. 명확한 책임 분리

- Domain: 비즈니스 로직
- Storage: 영속성
- Server: Use Case + API

### 2. 재사용성 향상

- Domain과 Storage는 모든 서버에서 공통 사용
- 중복 코드 제거

### 3. 테스트 용이성

- Domain은 순수 Java로 단위 테스트
- Storage는 Mock으로 테스트
- Server는 통합 테스트

### 4. 확장성

- 새로운 서버 추가 시 Domain/Storage 재사용
- 메시지 타입 추가 시 Domain만 수정

### 5. 유지보수성

- 각 계층의 변경이 다른 계층에 영향 최소화
- 코드 파악이 쉬움

---

## 🎯 다음 단계

1. ✅ 아키텍처 문서 작성 완료
2. ⬜ chat-domain 모듈 생성 및 구현
3. ⬜ chat-storage 리팩토링
4. ⬜ chat-message-server 리팩토링
5. ⬜ chat-system-server 리팩토링
6. ⬜ chat-websocket-server 리팩토링
7. ⬜ 통합 테스트 및 검증
8. ⬜ API 문서화

---

## 📚 참고 자료

- DDD (Domain-Driven Design)
- Hexagonal Architecture (Ports & Adapters)
- Clean Architecture
- CQRS Pattern
