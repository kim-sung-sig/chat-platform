# 채팅 플랫폼 마이그레이션 실행 계획

## 🎯 목표
현재 구조를 DDD 기반 멀티모듈 구조로 재구축

---

## 📋 현재 문제점

### 1. JPA 설정 문제
- `chat-storage`에 JPA가 있지만 다른 모듈에서 Entity를 인식하지 못함
- `@EntityScan`, `@EnableJpaRepositories` 설정 누락
- `chat-storage`가 라이브러리 모듈이지만 JPA Configuration이 각 서버에 분산

### 2. 도메인 로직 분산
- 도메인 서비스가 각 서버 모듈에 흩어져 있음
- 중복 코드 발생 가능성
- 비즈니스 로직 파악 어려움

### 3. Entity 미사용
- `chat-storage`의 Entity들이 실제로 사용되지 않음
- 각 서버에서 자체적으로 Repository 정의

### 4. 책임 불명확
- Domain Model과 Entity가 혼재
- 어디에 비즈니스 로직을 둬야 할지 불명확

---

## 🏗️ 새로운 구조

```
chat-platform/
├── common-util          (공통 유틸, 예외, 상수)
├── common-auth          (인증/인가)
├── common-logging       (로깅)
├── chat-domain          ⭐ NEW: 순수 도메인 + 인터페이스
│   ├── domain/
│   │   ├── channel/
│   │   ├── message/
│   │   ├── schedule/
│   │   └── user/
│   └── service/
├── chat-storage         ✏️ 수정: JPA 구현
│   ├── entity/
│   ├── repository/
│   ├── adapter/         ⭐ NEW: Repository 구현
│   └── mapper/          ⭐ NEW: Entity ↔ Domain 변환
├── chat-message-server  ✏️ 리팩토링
├── chat-system-server   ✏️ 리팩토링
└── chat-websocket-server ✏️ 리팩토링
```

---

## 📝 실행 단계

### ✅ Step 0: 준비 작업
- [x] 아키텍처 문서 작성
- [ ] 현재 코드 백업
- [ ] Git 브랜치 생성 (`feature/architecture-refactoring`)

---

### 🔹 Step 1: chat-domain 모듈 생성

#### 1.1 모듈 생성
```bash
mkdir -p chat-domain/src/main/java/com/example/chat/domain
mkdir -p chat-domain/src/test/java/com/example/chat/domain
```

#### 1.2 build.gradle 작성
```gradle
plugins {
    id 'java'
}

dependencies {
    implementation project(':common-util')
    compileOnly 'org.projectlombok:lombok'
    annotationProcessor 'org.projectlombok:lombok'
}
```

#### 1.3 도메인 모델 정의

**Channel Domain**
```
chat-domain/src/main/java/com/example/chat/domain/
├── channel/
│   ├── Channel.java              (Aggregate Root)
│   ├── ChannelId.java            (Value Object)
│   ├── ChannelType.java          (Enum)
│   ├── ChannelMember.java        (Entity)
│   ├── ChannelRepository.java    (Interface)
│   └── ChannelReader.java        (Interface - 읽기 전용)
```

**Message Domain**
```
├── message/
│   ├── Message.java              (Aggregate Root)
│   ├── MessageId.java            (Value Object)
│   ├── MessageType.java          (Enum: TEXT, IMAGE, FILE, SYSTEM)
│   ├── MessageStatus.java        (Enum: PENDING, SENT, DELIVERED, READ, FAILED)
│   ├── MessageContent.java       (Value Object)
│   ├── MessageRepository.java    (Interface)
│   └── MessageReader.java        (Interface - 읽기 전용)
```

**Schedule Domain**
```
├── schedule/
│   ├── ScheduleRule.java         (Aggregate Root)
│   ├── ScheduleId.java           (Value Object)
│   ├── ScheduleType.java         (Enum: ONE_TIME, RECURRING)
│   ├── ScheduleStatus.java       (Enum: PENDING, ACTIVE, EXECUTED, CANCELLED)
│   ├── CronExpression.java       (Value Object)
│   ├── ScheduleRuleRepository.java (Interface)
│   └── ScheduleReader.java       (Interface - 읽기 전용)
```

**User Domain**
```
└── user/
    ├── User.java                 (Aggregate Root)
    ├── UserId.java               (Value Object)
    └── UserRepository.java       (Interface)
```

#### 1.4 도메인 서비스 정의
```
chat-domain/src/main/java/com/example/chat/domain/service/
├── MessageDomainService.java
├── ChannelDomainService.java
└── ScheduleDomainService.java
```

#### 1.5 공통 Value Objects
```
chat-domain/src/main/java/com/example/chat/domain/common/
├── Cursor.java
├── PageResult.java
└── DomainEvent.java
```

---

### 🔹 Step 2: chat-storage 리팩토링

#### 2.1 디렉토리 구조 정리
```
chat-storage/src/main/java/com/example/chat/storage/
├── entity/                    ✏️ 기존 Entity 정리
│   ├── ChatChannelEntity.java
│   ├── ChatMessageEntity.java
│   ├── MessageReadEntity.java
│   ├── ScheduleRuleEntity.java
│   └── OutboxEventEntity.java
├── repository/                ✏️ JPA Repository
│   ├── JpaChatChannelRepository.java
│   ├── JpaChatMessageRepository.java
│   ├── JpaMessageReadRepository.java
│   └── JpaScheduleRuleRepository.java
├── adapter/                   ⭐ NEW: Domain Repository 구현
│   ├── ChannelRepositoryAdapter.java
│   ├── MessageRepositoryAdapter.java
│   ├── ScheduleRepositoryAdapter.java
│   ├── ChannelReaderAdapter.java
│   └── MessageReaderAdapter.java
├── mapper/                    ⭐ NEW: 변환 로직
│   ├── ChannelMapper.java
│   ├── MessageMapper.java
│   └── ScheduleMapper.java
└── config/
    └── JpaConfig.java         ✏️ 수정
```

#### 2.2 Entity에 Enum 추가
```java
// MessageStatus.java (chat-domain으로 이동)
public enum MessageStatus {
    PENDING,
    SENT,
    DELIVERED,
    READ,
    FAILED
}

// ScheduleStatus.java (chat-domain으로 이동)
public enum ScheduleStatus {
    PENDING,
    ACTIVE,
    EXECUTED,
    CANCELLED,
    FAILED
}

// ScheduleType.java (chat-domain으로 이동)
public enum ScheduleType {
    ONE_TIME,
    RECURRING
}
```

#### 2.3 Adapter 구현 예시
```java
@Repository
@RequiredArgsConstructor
public class MessageRepositoryAdapter implements MessageRepository {
    private final JpaChatMessageRepository jpaRepository;
    private final MessageMapper mapper;
    
    @Override
    public Message save(Message message) {
        ChatMessageEntity entity = mapper.toEntity(message);
        ChatMessageEntity saved = jpaRepository.save(entity);
        return mapper.toDomain(saved);
    }
    
    @Override
    public Optional<Message> findById(MessageId id) {
        return jpaRepository.findById(id.getValue())
            .map(mapper::toDomain);
    }
    
    @Override
    public List<Message> findByChannelId(ChannelId channelId, Cursor cursor, int limit) {
        // QueryDSL을 활용한 커서 기반 페이징
        // ...
    }
}
```

#### 2.4 JpaConfig 수정
```java
@Configuration
@EnableJpaRepositories(basePackages = "com.example.chat.storage.repository")
@EntityScan(basePackages = "com.example.chat.storage.entity")
public class JpaConfig {
    // JPA 설정
}
```

#### 2.5 build.gradle 수정
```gradle
dependencies {
    implementation project(':common-util')
    implementation project(':chat-domain')  // ⭐ 추가
    
    implementation 'org.springframework.boot:spring-boot-starter-data-jpa'
    implementation 'org.springframework.boot:spring-boot-starter-validation'
    runtimeOnly 'org.postgresql:postgresql'
    
    // QueryDSL
    implementation "com.querydsl:querydsl-jpa:${querydslVersion}:jakarta"
    annotationProcessor "com.querydsl:querydsl-apt:${querydslVersion}:jakarta"
}
```

---

### 🔹 Step 3: chat-message-server 리팩토링

#### 3.1 패키지 구조 변경
```
chat-message-server/src/main/java/com/example/chat/message/
├── application/               ⭐ NEW: Use Case
│   ├── SendMessageUseCase.java
│   ├── PublishMessageUseCase.java
│   └── MarkMessageReadUseCase.java
├── presentation/              ✏️ 기존 controller 이동
│   ├── MessageController.java
│   ├── MessageReadController.java
│   └── dto/
│       ├── request/
│       └── response/
├── infrastructure/            ✏️ 기존 redis, event
│   ├── redis/
│   │   └── RedisMessagePublisher.java
│   └── event/
│       └── MessageEventPublisher.java
└── config/
    ├── SecurityConfig.java
    └── RedisConfig.java
```

#### 3.2 Use Case 구현 (Early Return 패턴)
```java
@Service
@RequiredArgsConstructor
public class SendMessageUseCase {
    private final MessageRepository messageRepository;
    private final ChannelRepository channelRepository;
    private final MessageDomainService messageDomainService;
    private final RedisMessagePublisher redisPublisher;
    
    @Transactional
    public MessageResponse execute(SendMessageCommand command) {
        // 1. 채널 조회 (Key → Domain)
        Channel channel = channelRepository.findById(command.channelId())
            .orElseThrow(() -> new ChannelNotFoundException(command.channelId()));
        
        // 2. Early Return: 권한 검증
        if (!channel.isMember(command.senderId())) {
            throw new UnauthorizedAccessException("Not a member of channel");
        }
        
        // 3. Early Return: 채널 상태 검증
        if (!channel.isActive()) {
            throw new InactiveChannelException("Channel is not active");
        }
        
        // 4. 도메인 서비스로 메시지 생성
        Message message = messageDomainService.createMessage(
            channel.getId(),
            command.senderId(),
            command.content()
        );
        
        // 5. 저장
        Message saved = messageRepository.save(message);
        
        // 6. 이벤트 발행
        redisPublisher.publish(saved);
        
        // 7. 응답
        return MessageResponse.from(saved);
    }
}
```

#### 3.3 Controller는 얇게 유지
```java
@RestController
@RequestMapping("/api/v1/messages")
@RequiredArgsConstructor
public class MessageController {
    private final SendMessageUseCase sendMessageUseCase;
    
    @PostMapping
    public ResponseEntity<MessageResponse> sendMessage(
        @Valid @RequestBody SendMessageRequest request,
        @AuthenticationPrincipal UserPrincipal principal
    ) {
        SendMessageCommand command = SendMessageCommand.of(
            request,
            principal.getUserId()
        );
        
        MessageResponse response = sendMessageUseCase.execute(command);
        
        return ResponseEntity.ok(response);
    }
}
```

#### 3.4 build.gradle에 의존성 추가
```gradle
dependencies {
    implementation project(':common-util')
    implementation project(':common-auth')
    implementation project(':common-logging')
    implementation project(':chat-domain')    // ⭐ 추가
    implementation project(':chat-storage')   // ⭐ 추가
    
    // ... 기존 의존성
}
```

#### 3.5 application.yml에 JPA 설정 추가
```yaml
spring:
  jpa:
    hibernate:
      ddl-auto: validate
    show-sql: false
    properties:
      hibernate:
        format_sql: true
        default_batch_fetch_size: 100
```

---

### 🔹 Step 4: chat-system-server 리팩토링

#### 4.1 패키지 구조 변경
```
chat-system-server/src/main/java/com/example/chat/system/
├── application/               ⭐ NEW: Use Case
│   ├── channel/
│   │   ├── CreateChannelUseCase.java
│   │   ├── JoinChannelUseCase.java
│   │   └── LeaveChannelUseCase.java
│   ├── message/
│   │   └── QueryMessageUseCase.java (Cursor 기반)
│   └── schedule/
│       ├── CreateOneTimeScheduleUseCase.java
│       ├── CreateRecurringScheduleUseCase.java
│       ├── CancelScheduleUseCase.java
│       └── ForcePublishScheduleUseCase.java
├── presentation/              ✏️ Controller
│   ├── ChannelController.java
│   ├── MessageQueryController.java
│   ├── ScheduleController.java
│   └── dto/
├── infrastructure/
│   ├── scheduler/
│   │   ├── MessagePublishJob.java
│   │   ├── QuartzSchedulerService.java
│   │   └── ConcurrencyGuard.java
│   └── client/
│       └── MessageServerClient.java (REST 호출)
└── config/
    ├── QuartzConfig.java
    └── RestTemplateConfig.java
```

#### 4.2 Cursor 기반 메시지 조회
```java
@Service
@RequiredArgsConstructor
public class QueryMessageUseCase {
    private final MessageRepository messageRepository;
    private final ChannelRepository channelRepository;
    
    @Transactional(readOnly = true)
    public MessagePageResponse execute(QueryMessageCommand command) {
        // 1. 채널 조회
        Channel channel = channelRepository.findById(command.channelId())
            .orElseThrow(() -> new ChannelNotFoundException(command.channelId()));
        
        // 2. Early Return: 권한 검증
        if (!channel.isMember(command.userId())) {
            throw new UnauthorizedAccessException("Not a member");
        }
        
        // 3. Cursor 기반 조회 (Offset 기반 아님!)
        List<Message> messages = messageRepository.findByChannelId(
            command.channelId(),
            command.cursor(),
            command.limit()
        );
        
        // 4. 다음 커서 생성
        Cursor nextCursor = messages.isEmpty() ? null : 
            Cursor.from(messages.get(messages.size() - 1));
        
        return MessagePageResponse.of(messages, nextCursor, command.limit());
    }
}
```

#### 4.3 단발성 스케줄 등록
```java
@Service
@RequiredArgsConstructor
public class CreateOneTimeScheduleUseCase {
    private final ScheduleRuleRepository scheduleRepository;
    private final ChannelRepository channelRepository;
    private final MessageDomainService messageDomainService;
    private final ScheduleDomainService scheduleDomainService;
    private final QuartzSchedulerService quartzService;
    
    @Transactional
    public ScheduleResponse execute(CreateOneTimeScheduleCommand command) {
        // 1. 채널 조회
        Channel channel = channelRepository.findById(command.channelId())
            .orElseThrow(() -> new ChannelNotFoundException(command.channelId()));
        
        // 2. Early Return: 권한 검증
        if (!channel.isMember(command.senderId())) {
            throw new UnauthorizedAccessException("Not a member");
        }
        
        // 3. Early Return: 시간 검증
        if (command.scheduledAt().isBefore(Instant.now())) {
            throw new InvalidScheduleTimeException("Cannot schedule in the past");
        }
        
        // 4. 메시지 생성 (도메인 서비스)
        Message message = messageDomainService.createMessage(
            command.channelId(),
            command.senderId(),
            command.content()
        );
        
        // 5. 스케줄 생성 (도메인 서비스)
        ScheduleRule schedule = scheduleDomainService.createOneTimeSchedule(
            message,
            command.scheduledAt()
        );
        
        // 6. 저장
        ScheduleRule saved = scheduleRepository.save(schedule);
        
        // 7. Quartz Job 등록 (SimpleTrigger)
        quartzService.scheduleOneTimeJob(saved);
        
        return ScheduleResponse.from(saved);
    }
}
```

#### 4.4 주기적 스케줄 등록
```java
@Service
@RequiredArgsConstructor
public class CreateRecurringScheduleUseCase {
    private final ScheduleRuleRepository scheduleRepository;
    private final ChannelRepository channelRepository;
    private final MessageDomainService messageDomainService;
    private final ScheduleDomainService scheduleDomainService;
    private final QuartzSchedulerService quartzService;
    
    @Transactional
    public ScheduleResponse execute(CreateRecurringScheduleCommand command) {
        // 1. 채널 조회
        Channel channel = channelRepository.findById(command.channelId())
            .orElseThrow(() -> new ChannelNotFoundException(command.channelId()));
        
        // 2. Early Return: 권한 검증
        if (!channel.isMember(command.senderId())) {
            throw new UnauthorizedAccessException("Not a member");
        }
        
        // 3. 메시지 템플릿 생성
        Message messageTemplate = messageDomainService.createMessage(
            command.channelId(),
            command.senderId(),
            command.content()
        );
        
        // 4. 주기 스케줄 생성
        ScheduleRule schedule = scheduleDomainService.createRecurringSchedule(
            messageTemplate,
            command.cronExpression()
        );
        
        // 5. 저장
        ScheduleRule saved = scheduleRepository.save(schedule);
        
        // 6. Quartz Cron Job 등록
        quartzService.scheduleRecurringJob(saved);
        
        return ScheduleResponse.from(saved);
    }
}
```

#### 4.5 강제 발송 (동시성 제어)
```java
@Service
@RequiredArgsConstructor
public class ForcePublishScheduleUseCase {
    private final ScheduleRuleRepository scheduleRepository;
    private final MessageServerClient messageClient;
    private final QuartzSchedulerService quartzService;
    
    @Transactional
    public void execute(ForcePublishCommand command) {
        // 1. 비관적 락으로 조회 (동시성 제어)
        ScheduleRule schedule = scheduleRepository
            .findByIdWithLock(command.scheduleId())
            .orElseThrow(() -> new ScheduleNotFoundException(command.scheduleId()));
        
        // 2. Early Return: 상태 검증
        if (!schedule.canBeExecuted()) {
            throw new InvalidScheduleStateException(
                "Schedule is not in executable state: " + schedule.getStatus()
            );
        }
        
        // 3. 단발성인 경우 Quartz Job 삭제
        if (schedule.isOneTime()) {
            quartzService.deleteJob(schedule.getId());
            schedule.markAsExecuted();
        }
        // 주기적인 경우 Job은 유지 (다음 실행 대기)
        
        // 4. 메시지 발송 (chat-message-server REST 호출)
        messageClient.sendMessage(schedule.getMessage());
        
        // 5. 저장
        scheduleRepository.save(schedule);
    }
}
```

#### 4.6 Quartz Job 구현 (동시성 제어)
```java
@Component
@DisallowConcurrentExecution  // ⭐ 중요: 동시 실행 방지
@RequiredArgsConstructor
public class MessagePublishJob implements Job {
    private final ScheduleRuleRepository scheduleRepository;
    private final MessageServerClient messageClient;
    
    @Override
    public void execute(JobExecutionContext context) throws JobExecutionException {
        String scheduleId = context.getJobDetail().getKey().getName();
        
        try {
            // 1. 비관적 락으로 조회
            ScheduleRule schedule = scheduleRepository
                .findByIdWithLock(ScheduleId.of(scheduleId))
                .orElseThrow(() -> new ScheduleNotFoundException(scheduleId));
            
            // 2. Early Return: 상태 검증
            if (!schedule.canBeExecuted()) {
                return;
            }
            
            // 3. 메시지 발송
            messageClient.sendMessage(schedule.getMessage());
            
            // 4. 단발성인 경우 상태 업데이트
            if (schedule.isOneTime()) {
                schedule.markAsExecuted();
                scheduleRepository.save(schedule);
            }
            // 주기적인 경우 상태는 유지
            
        } catch (Exception e) {
            throw new JobExecutionException("Failed to publish message", e);
        }
    }
}
```

#### 4.7 QuartzSchedulerService
```java
@Service
@RequiredArgsConstructor
public class QuartzSchedulerService {
    private final Scheduler scheduler;
    
    public void scheduleOneTimeJob(ScheduleRule schedule) {
        JobDetail jobDetail = JobBuilder.newJob(MessagePublishJob.class)
            .withIdentity(schedule.getId().getValue())
            .usingJobData("scheduleId", schedule.getId().getValue())
            .build();
        
        Trigger trigger = TriggerBuilder.newTrigger()
            .withIdentity(schedule.getId().getValue() + "-trigger")
            .startAt(Date.from(schedule.getScheduledAt()))
            .build();
        
        scheduler.scheduleJob(jobDetail, trigger);
    }
    
    public void scheduleRecurringJob(ScheduleRule schedule) {
        JobDetail jobDetail = JobBuilder.newJob(MessagePublishJob.class)
            .withIdentity(schedule.getId().getValue())
            .usingJobData("scheduleId", schedule.getId().getValue())
            .storeDurably()  // Job 유지
            .build();
        
        CronTrigger trigger = TriggerBuilder.newTrigger()
            .withIdentity(schedule.getId().getValue() + "-trigger")
            .withSchedule(CronScheduleBuilder.cronSchedule(schedule.getCronExpression().getValue()))
            .build();
        
        scheduler.scheduleJob(jobDetail, trigger);
    }
    
    public void deleteJob(ScheduleId scheduleId) {
        JobKey jobKey = JobKey.jobKey(scheduleId.getValue());
        scheduler.deleteJob(jobKey);
    }
}
```

---

### 🔹 Step 5: chat-websocket-server 리팩토링

#### 5.1 패키지 구조
```
chat-websocket-server/src/main/java/com/example/chat/websocket/
├── application/
│   └── WebSocketMessageHandler.java
├── infrastructure/
│   ├── session/
│   │   └── WebSocketSessionManager.java (Redis 기반)
│   └── subscriber/
│       └── RedisMessageSubscriber.java
└── config/
    ├── WebSocketConfig.java
    └── RedisSubscriberConfig.java
```

#### 5.2 Redis 기반 세션 관리 (멀티 인스턴스 대응)
```java
@Component
@RequiredArgsConstructor
public class WebSocketSessionManager {
    private final RedisTemplate<String, String> redisTemplate;
    
    // Redis에 세션 정보 저장
    // Key: "ws:channel:{channelId}:sessions"
    // Value: Set<sessionId>
    
    public void registerSession(String sessionId, ChannelId channelId) {
        String key = "ws:channel:" + channelId.getValue() + ":sessions";
        redisTemplate.opsForSet().add(key, sessionId);
    }
    
    public void unregisterSession(String sessionId, ChannelId channelId) {
        String key = "ws:channel:" + channelId.getValue() + ":sessions";
        redisTemplate.opsForSet().remove(key, sessionId);
    }
    
    public Set<String> getSessionsByChannel(ChannelId channelId) {
        String key = "ws:channel:" + channelId.getValue() + ":sessions";
        return redisTemplate.opsForSet().members(key);
    }
}
```

---

### 🔹 Step 6: 불필요한 파일 정리

#### 6.1 .bak 파일 삭제
```bash
# .bak 파일 찾기 및 삭제
find . -name "*.bak" -type f -delete
```

#### 6.2 사용하지 않는 패키지/클래스 삭제
- [ ] chat-storage의 handler 패키지 (factory 패턴은 domain으로 이동)
- [ ] 각 서버의 domain 패키지 (chat-domain으로 이동)
- [ ] 중복된 DTO 클래스

---

### 🔹 Step 7: settings.gradle 수정

```gradle
rootProject.name = 'chat-platform'

include 'common-util'
include 'common-auth'
include 'common-logging'

include 'chat-domain'          // ⭐ 추가
include 'chat-storage'

include 'chat-message-server'
include 'chat-system-server'
include 'chat-websocket-server'
```

---

### 🔹 Step 8: 테스트 및 검증

#### 8.1 컴파일 확인
```bash
./gradlew clean build -x test
```

#### 8.2 단위 테스트
```bash
./gradlew test
```

#### 8.3 통합 테스트
- [ ] chat-message-server 기동 확인
- [ ] chat-system-server 기동 확인
- [ ] chat-websocket-server 기동 확인
- [ ] API 테스트
- [ ] 스케줄러 동작 확인

---

## 📊 동시성 제어 전략 정리

### 단발성 메시지 (ONE_TIME)
1. **DB 비관적 락**: `SELECT ... FOR UPDATE`
2. **상태 검증**: `ScheduleStatus.PENDING` → `EXECUTED`
3. **Quartz Job 삭제**: 실행 후 Job 제거
4. **메시지 상태**: Message의 `MessageStatus`로 중복 발송 방지

### 주기적 메시지 (RECURRING)
1. **@DisallowConcurrentExecution**: Quartz 레벨 동시 실행 방지
2. **DB 비관적 락**: Job 실행 시 ScheduleRule 락 획득
3. **메시지 새로 생성**: 매번 새로운 MessageId로 생성 (중복 없음)
4. **Job 유지**: CronTrigger가 계속 실행

### 강제 발송 vs 자동 발송 충돌
```
강제 발송 (ForcePublishScheduleUseCase)
    ↓
1. ScheduleRule 비관적 락 획득
    ↓
2. 상태 검증 (Early Return)
    ↓
3. 단발성인 경우 Quartz Job 삭제
    ↓
4. 메시지 발송
    ↓
5. 상태 업데이트 → EXECUTED
    ↓
6. 락 해제

자동 발송 (MessagePublishJob)
    ↓
1. ScheduleRule 비관적 락 시도
    ↓
    (강제 발송이 락을 잡고 있으면 대기)
    ↓
2. 락 획득 후 상태 확인
    ↓
3. 이미 EXECUTED면 Early Return (중복 방지)
```

---

## 🎯 체크리스트

### Phase 1: Domain 계층
- [ ] chat-domain 모듈 생성
- [ ] Channel Aggregate 정의
- [ ] Message Aggregate 정의
- [ ] ScheduleRule Aggregate 정의
- [ ] Repository Interface 정의
- [ ] Domain Service 구현
- [ ] Enum 정의 (MessageStatus, ScheduleStatus, ScheduleType 등)

### Phase 2: Storage 계층
- [ ] chat-storage 리팩토링
- [ ] Entity에 Enum 적용
- [ ] Adapter 구현
- [ ] Mapper 구현
- [ ] JpaConfig 수정
- [ ] QueryDSL 설정

### Phase 3: Server 모듈
- [ ] chat-message-server Use Case 구현
- [ ] chat-system-server Use Case 구현
- [ ] chat-websocket-server Redis 세션 관리
- [ ] Early Return 패턴 적용
- [ ] 코드 컨벤션 준수

### Phase 4: 테스트
- [ ] 단위 테스트
- [ ] 통합 테스트
- [ ] API 테스트
- [ ] 동시성 테스트

### Phase 5: 정리
- [ ] .bak 파일 삭제
- [ ] 사용하지 않는 코드 제거
- [ ] 문서화

---

## 🚀 다음 단계

이제 실제 구현을 시작합니다!

```
1. ✅ 아키텍처 문서 작성 완료
2. ✅ 마이그레이션 계획 작성 완료
3. ⬜ Step 1: chat-domain 모듈 생성  ← 여기서부터 시작
```
