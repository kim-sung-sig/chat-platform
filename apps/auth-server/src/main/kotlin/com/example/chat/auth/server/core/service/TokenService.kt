...
@Service
class TokenService(
    @Value("\${auth.jwt.secret:your-256-bit-secret-key-change-this-in-production-environment}") private val secretKey: String,
    ...
    fun createFullAccessToken(principalId: UUID, identifier: String, authLevel: AuthLevel, device: Device): Token {
        val now = Instant.now()
        ...
        val accessToken = buildJwt(principalId, identifier, authLevel, TokenType.FULL_ACCESS, expiresAt)
        val refreshTokenVal = buildJwt(principalId, identifier, authLevel, TokenType.FULL_ACCESS, refreshExpiresAt)
    ...
    fun createMfaPendingToken(principalId: UUID, identifier: String, currentLevel: AuthLevel, mfaSessionId: String): Token {
        val now = Instant.now()
        val expiresAt = now.plus(mfaTokenTtlMinutes, ChronoUnit.MINUTES)

        val accessToken = buildMfaJwt(principalId, identifier, currentLevel, mfaSessionId, expiresAt)
    ...
    private fun buildJwt(principalId: UUID, identifier: String, authLevel: AuthLevel, tokenType: TokenType, expiresAt: Instant): String {
        return try {
            ...
            throw RuntimeException("Failed to create JWT", e)
        }
    }

    private fun buildMfaJwt(principalId: UUID, identifier: String, authLevel: AuthLevel, mfaSessionId: String, expiresAt: Instant): String {
        return try {
            val claimsSet = JWTClaimsSet.Builder()
                .subject(principalId.toString())
                .claim("identifier", identifier)
            ...
            throw RuntimeException("Failed to create MFA JWT", e)
        }
    }
    ...
    fun rotateRefreshToken(refreshTokenVal: String, device: Device): Token {
        ...
        val refreshToken = refreshTokenRepository.findByTokenValue(refreshTokenVal)
        ...
        if (!refreshToken.device.isSameDevice(device)) {
            throw RuntimeException("Device mismatch")
        }
