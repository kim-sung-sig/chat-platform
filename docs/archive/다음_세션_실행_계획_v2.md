# 다음 세션 실행 계획

## 📅 계획 일자: 2025-12-13

---

## 🎯 세션 목표: Channel 관리 & 메시지 조회 기능 구현

---

## 📋 Step 1: Channel 관리 기능 구현

### 1.1 ChannelApplicationService 생성

**위치:** `chat-system-server/src/main/java/com/example/chat/system/application/service/ChannelApplicationService.java`

**구현 메서드:**

```java
// 채널 생성
public ChannelResponse createDirectChannel(CreateDirectChannelRequest request)
public ChannelResponse createGroupChannel(CreateGroupChannelRequest request)
public ChannelResponse createPublicChannel(CreatePublicChannelRequest request)
public ChannelResponse createPrivateChannel(CreatePrivateChannelRequest request)

// 채널 멤버 관리
public void addMemberToChannel(String channelId, String userId)
public void removeMemberFromChannel(String channelId, String userId)

// 채널 조회
public ChannelResponse getChannel(String channelId)
public List<ChannelResponse> getMyChannels()
public List<ChannelResponse> getPublicChannels()

// 채널 수정
public ChannelResponse updateChannelInfo(String channelId, UpdateChannelRequest request)
public void deactivateChannel(String channelId)
```

**DDD 패턴 적용:**

1. Key로 Aggregate 조회 (Channel, User)
2. ChannelDomainService 호출 (Aggregate 전달)
3. 영속화
4. 이벤트 발행 (선택)
5. DTO 변환

---

### 1.2 DTO 생성

**Request DTOs:**

- `CreateDirectChannelRequest` (user1Id, user2Id)
- `CreateGroupChannelRequest` (name, memberIds)
- `CreatePublicChannelRequest` (name, description)
- `CreatePrivateChannelRequest` (name, description, memberIds)
- `UpdateChannelRequest` (name, description)

**Response DTOs:**

- `ChannelResponse` (id, name, type, ownerId, memberCount, active, createdAt)
- `ChannelDetailResponse` (extends ChannelResponse + memberIds)

---

### 1.3 Controller 생성

**위치:** `chat-system-server/src/main/java/com/example/chat/system/controller/ChannelController.java`

**엔드포인트:**

```
POST   /api/v1/channels/direct
POST   /api/v1/channels/group
POST   /api/v1/channels/public
POST   /api/v1/channels/private
GET    /api/v1/channels/{channelId}
GET    /api/v1/channels/my
GET    /api/v1/channels/public
PUT    /api/v1/channels/{channelId}
DELETE /api/v1/channels/{channelId}
POST   /api/v1/channels/{channelId}/members
DELETE /api/v1/channels/{channelId}/members/{userId}
```

---

## 📋 Step 2: 메시지 조회 기능 구현

### 2.1 커서 기반 페이징 구현

**MessageRepository 확장:**

```java
// 커서 기반 페이징
List<Message> findByChannelIdWithCursor(
    ChannelId channelId, 
    Cursor cursor, 
    int limit
);

// 특정 메시지 이전/이후 조회
List<Message> findByChannelIdBeforeMessage(
    ChannelId channelId, 
    MessageId messageId, 
    int limit
);

List<Message> findByChannelIdAfterMessage(
    ChannelId channelId, 
    MessageId messageId, 
    int limit
);
```

**Storage Layer 구현:**

- `JpaMessageRepository`에 커서 기반 쿼리 추가
- `@Query` 어노테이션 활용

---

### 2.2 MessageQueryService 생성

**위치:** `chat-system-server/src/main/java/com/example/chat/system/application/service/MessageQueryService.java`

**구현 메서드:**

```java
// 메시지 조회 (커서 기반 페이징)
public CursorPageResponse<MessageResponse> getMessages(
    String channelId, 
    String cursor, 
    int limit
);

// 특정 메시지 조회
public MessageResponse getMessage(String messageId);

// 읽지 않은 메시지 수 조회
public long getUnreadMessageCount(String channelId);
```

**커서 생성 방식:**

```java
// Cursor = Base64(messageId + ":" + createdAt.toEpochMilli())
String cursor = Base64.encode(message.getId() + ":" + message.getCreatedAt().toEpochMilli());
```

---

### 2.3 Controller 생성

**위치:** `chat-system-server/src/main/java/com/example/chat/system/controller/MessageQueryController.java`

**엔드포인트:**

```
GET /api/v1/messages?channelId={channelId}&cursor={cursor}&limit={limit}
GET /api/v1/messages/{messageId}
GET /api/v1/messages/unread-count?channelId={channelId}
```

---

## 📋 Step 3: WebSocket Server 리팩토링

### 3.1 Domain 모델 적용

**기존 문제:**

- chat-websocket-server가 Domain 모델을 사용하지 않음
- 독자적인 DTO 사용

**개선:**

- `chat-domain` 모듈 의존성 추가
- Domain 모델 활용 (Message, Channel 등)

---

### 3.2 Redis Pub/Sub 연동 확인

**흐름:**

1. chat-message-server에서 메시지 발행 (Redis Pub)
2. chat-websocket-server(여러 인스턴스)에서 메시지 수신 (Redis Sub)
3. 연결된 WebSocket 세션에 메시지 전송

**확인 사항:**

- RedisMessageListener 정상 동작 확인
- 멀티 인스턴스 환경에서 중복 전송 방지

---

### 3.3 ChatRoomSessionManager 리팩토링

**기존 문제:**

- 로컬 메모리 기반 (멀티 인스턴스 대응 불가)

**개선:**

- Redis 기반 Session 관리
- `RedisTemplate`을 활용한 채널별 세션 관리
- TTL 설정 (세션 만료 처리)

**구현:**

```java
public class RedisChatRoomSessionManager {
    private final RedisTemplate<String, Set<String>> redisTemplate;
    
    public void addSession(String channelId, String sessionId) {
        String key = "channel:sessions:" + channelId;
        redisTemplate.opsForSet().add(key, sessionId);
        redisTemplate.expire(key, 24, TimeUnit.HOURS);
    }
    
    public Set<String> getSessions(String channelId) {
        String key = "channel:sessions:" + channelId;
        return redisTemplate.opsForSet().members(key);
    }
    
    public void removeSession(String channelId, String sessionId) {
        String key = "channel:sessions:" + channelId;
        redisTemplate.opsForSet().remove(key, sessionId);
    }
}
```

---

## 📋 Step 4: 통합 테스트 작성

### 4.1 Domain Service 단위 테스트

**위치:** `chat-domain/src/test/java/com/example/chat/domain/service/`

**테스트 케이스:**

```java
// MessageDomainServiceTest
@Test
void createTextMessage_success()
@Test
void createTextMessage_channelNotActive_throwsException()
@Test
void createTextMessage_userNotMember_throwsException()
@Test
void createTextMessage_userBanned_throwsException()

// ChannelDomainServiceTest
@Test
void createDirectChannel_success()
@Test
void createDirectChannel_sameUser_throwsException()
@Test
void addMemberToChannel_success()
@Test
void addMemberToChannel_alreadyMember_throwsException()
```

---

### 4.2 Application Service 통합 테스트

**위치:** `chat-system-server/src/test/java/com/example/chat/system/application/service/`

**테스트 설정:**

- `@SpringBootTest`
- `@Transactional`
- Test Container (PostgreSQL, Redis)

**테스트 케이스:**

```java
// ChannelApplicationServiceTest
@Test
void createDirectChannel_success()
@Test
void addMemberToChannel_success()

// MessageQueryServiceTest
@Test
void getMessages_cursorPagination_success()
```

---

### 4.3 API 엔드투엔드 테스트

**위치:** `chat-system-server/src/test/java/com/example/chat/system/controller/`

**테스트 설정:**

- `@WebMvcTest` 또는 `@SpringBootTest(webEnvironment = RANDOM_PORT)`
- MockMvc 또는 RestAssured

**테스트 케이스:**

```java
// ChannelControllerTest
@Test
void createDirectChannel_validRequest_returns201()
@Test
void createDirectChannel_invalidRequest_returns400()

// MessageQueryControllerTest
@Test
void getMessages_validCursor_returns200()
```

---

## 📋 Step 5: 문서화 및 정리

### 5.1 API 문서 작성

- Swagger/OpenAPI 설정 확인
- 각 API 엔드포인트 예시 요청/응답 추가

### 5.2 README 업데이트

- 프로젝트 구조 설명
- 실행 방법
- API 문서 링크

### 5.3 불필요한 파일 정리

- `.bak` 파일 제거
- 사용하지 않는 패키지/클래스 제거

---

## ✅ 체크리스트

### Domain Service

- [x] MessageDomainService 리팩토링
- [x] ChannelDomainService 리팩토링
- [x] ScheduleDomainService 리팩토링
- [x] Early Return 패턴 적용
- [x] Aggregate 기반 협력

### Application Service

- [x] MessageApplicationService 구현
- [x] ScheduleService 리팩토링
- [ ] ChannelApplicationService 구현
- [ ] MessageQueryService 구현

### Controller

- [ ] ChannelController 구현
- [ ] MessageQueryController 구현

### Storage

- [ ] MessageRepository 커서 페이징 구현
- [ ] ChannelRepository 확장

### WebSocket

- [ ] Domain 모델 적용
- [ ] Redis Session Manager 구현
- [ ] 멀티 인스턴스 대응

### 테스트

- [ ] Domain Service 단위 테스트
- [ ] Application Service 통합 테스트
- [ ] Controller API 테스트

---

## 🚀 예상 소요 시간

- Step 1: Channel 관리 기능 - **2시간**
- Step 2: 메시지 조회 기능 - **1.5시간**
- Step 3: WebSocket Server 리팩토링 - **1.5시간**
- Step 4: 통합 테스트 - **2시간**
- Step 5: 문서화 및 정리 - **1시간**

**총 예상 시간: 8시간**

---

## 💡 주의사항

1. **DDD 패턴 준수:**
	- Application Service는 Aggregate 조회 + Domain Service 호출
	- Domain Service는 Aggregate 협력 조율
	- Early Return 패턴 일관 적용

2. **코드 컨벤션:**
	- 주석 작성 (도메인 규칙 명확히)
	- 입력값 검증 (Early Validation)
	- 책임 명확화 (Single Responsibility)

3. **테스트 작성:**
	- 핵심 비즈니스 로직은 반드시 테스트
	- Given-When-Then 패턴 사용
	- 의미 있는 테스트명

4. **성능 고려:**
	- 커서 기반 페이징 (Offset 페이징 지양)
	- N+1 문제 방지 (Fetch Join 활용)
	- Redis 캐싱 고려

---

**작성자:** GitHub Copilot  
**상태:** 준비 완료  
**시작 예정:** 다음 세션
